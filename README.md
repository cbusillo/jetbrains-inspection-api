...
- For `current_file`, `files`, and `changed_files` scopes, the plugin first converts globally enabled local tools into exact tool/file obligations. Profile-disabled and positively language-inapplicable tools are excluded; applicable batch-runnable tools execute in parallel by exact file with isolated copied wrappers, independent inspection contexts, and one shared 25-file/60-second deadline. Every copied wrapper is initialized and cleaned up around its execution, and all workers finish or cancel before proof returns. Applicable non-batch tools, unresolved keys or wrappers, execution failures, unvisited obligations, and unmapped descriptors keep clean proof `UNKNOWN`/`execution_not_proven`. Current mapped findings remain decisive `RED` even when clean proof is incomplete. Exact-scope diagnostics use `execution_proof_mode: "exact_bounded"` and report the globally enabled count separately from candidate, applicable, non-applicable, runnable, executed, failed, unclassified, and unvisited obligation counts, with bounded blocking and non-applicable examples.
