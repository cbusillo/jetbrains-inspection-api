...
- For a selected Python scope with a real executable at the project-root `.venv`, the plugin gives PyCharm an initial ten-second window to register, assign, and finish updating that SDK before stable input fingerprinting. Registration and module/project assignment are observed separately; each forward transition can extend the bounded wait, up to thirty seconds total, and readiness still requires two consecutive ready observations. A missing or invalid local interpreter fails immediately, a valid candidate that never registers remains terminal `language_sdk_missing`, and a registered SDK whose assignment is still propagating remains `project_analysis_not_ready`. Debug payloads expose registration, assignment, deadline-extension, and `python_sdk_settle_*` evidence; the plugin never writes JetBrains' global SDK table.
