...
- The supported `InspectionEngine.inspectEx` path is fail-closed for local `buildVisitor` construction failures on the tested IntelliJ Platform: the exception escapes instead of yielding an empty clean result, and exact bounded proof remains `UNKNOWN` with no executable batch obligation. Broad native execution that aborts also remains unproven, even after partial completion events. This is a platform-return guarantee, not a claim that the plugin can independently detect a future platform version that logs a construction failure and substitutes `PsiElementVisitor.EMPTY_VISITOR` without exposing a supported failure signal.
