...
- For a selected Python scope with a real executable at the project-root `.venv`, the plugin gives PyCharm a bounded five-second window to register, assign, and finish updating that SDK before stable input fingerprinting. Readiness requires two consecutive ready observations. A missing or invalid local interpreter still fails immediately; an unassigned candidate after the deadline remains terminal `language_sdk_missing`, while an assigned SDK that is still updating remains `project_analysis_not_ready`. Debug payloads expose `python_sdk_settle_*` timing and observation evidence; the plugin never writes JetBrains' global SDK table.
