...
- Scoped findings from completed exact bounded proof participate in deduplicated result settling even when a cold Inspection Results tool window is empty. Finding-bearing capture retains the existing 15-second result wait and five-second stability requirement; no-evidence capture remains fail-closed on the existing fallback path.
- For `whole_project` and `directory`, GREEN requires native-run attestation from the request's `GlobalInspectionContext` plus a run-bounded subscription to the JetBrains inspection event topic. The synchronous run must traverse at least one physical scope file, complete at least one applicable local or global-simple file inspection, return normally, report no inspection failures or unmapped problem counts, and survive the existing run/session/scope/profile/input freshness checks. Global-only completion or lifecycle activity without file traversal is not proof. A settled empty Problems/Inspection Results presentation remains insufficient by itself. Real findings remain RED; missing, aborted, failed, zero-file, or zero-file-scoped-tool attestation remains `UNKNOWN`/`execution_not_proven`. Broad-scope diagnostics use `execution_proof_mode: "native_attested"` plus native completion, file, aggregate inspection completion, distinct tool, problem, and failure counts. Per-kind completion and activity counters are no longer emitted; they were diagnostic only and never qualified a clean verdict. Targeted bounded scopes use the exact obligation counts described above.
- Broad-scope captures also include `native_tool_completion_observation` (schema 1, `mode: "report_only"`), pinned to `inspection_run_id`. It compares post-run profile/PSI candidate tool/file pairs with observed finishes by wrapper identity and file, and records language/scope exclusions and bounded examples. Candidate enumeration has a two-second cooperative budget, 200,000 PSI-element limit, and 100,000 candidate/completion limit; partial observations have no `candidate_rule_would_block_clean` comparison. It does not rerun inspections, alter profiles, or participate in execution proof, verdicts, or retry decisions. Missing finishes are not failures: empty visitors and other silent skips remain unobservable. Local events can overlap other project inspections; injected files, external annotators, and aggregate hooks are not modeled. A fully matched observation is not additional proof that every enabled inspection ran.
...
- `snapshot_change_kind` explains freshness classification when present. `snapshot_predates_current_trigger` and `unsaved_documents` are stale. `current_run_psi_churn` means the plugin proved that a PSI tick was benign for the originating inspection; `files` captures must select files covered by the content tracker; excluded and outside-root files retain their stale baseline. `files` and `changed_files` captures require unchanged inspection inputs and authoritative scoped results; an exact-file execution proof completed entirely in smart mode can establish the `files` result without an Inspection Results window; `changed_files` also revalidates its resolved file set. Indexing transitions can advance the PSI counter without editing source files. The plugin reconciles that counter only while publishing the originating run after validating project content, roots, SDKs, profiles, and results. Later unverified changes still make the snapshot stale.
...
reviewer), which is the only place `PUBLISH_TOKEN` exists, has read-only repository permission, runs no Gradle tasks, and uploads through the bounded
