...
Broad native execution no longer initializes or polls `InspectionResultsView`. Direct presentation descriptors remain the primary model, and native or bounded execution proof can confirm a readable empty model without a view. Stable successful tool-window extraction remains an alternate empty-result observation when available. Missing proof or an unreadable model remains `UNKNOWN`. The generic Problems/inspection tool-window fallback stays available, but the capture path no longer depends on `GlobalInspectionContextImpl.getView()` or `initializeViewIfNeeded()`.
...
- unpaired unfair wrappers with no batch wrapper → platform metadata classifies them as intentionally non-batch; engine integration must exclude them from the batch denominator and never count them as execution or clean evidence
